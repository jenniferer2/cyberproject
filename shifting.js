class Shifting {
  constructor(r, c) {
    // Constructor initializes the array and sets the number of rows and columns
  ...
  changeLetter(message) {
    // Shifts each letter in the message by one position to the right
  ...
  fillBlock(n) {
    // Fills the array with ! if the index of teh array is empty from the given message
    let i = 0;
  ...
  encryptOne() {
    // Performs one round of encryption on the array by shifting elements
  ...
  removed(d) {
    // Removes trailing '!' characters from the decrypted message
    let newn = d;
  ...
  returnToLetter(message) {
    // Shifts each letter in the message by one position to the left
    const alpha = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    let res = "";
